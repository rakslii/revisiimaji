...
<div {{ $attributes->merge(['class' => 'group']) }}>
    <div class="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 h-full flex flex-col hover:border-blue-200">
        <!-- Product Image -->
        <div class="relative overflow-hidden h-48 bg-gradient-to-br from-blue-50 to-gray-50">
            ...
            @else
                <div class="w-full h-full flex items-center justify-center p-4">
                    <i class="fas fa-print text-blue-200 text-6xl group-hover:text-blue-300 transition-colors"></i>
                </div>
            @endif
            ...
            @if($product->category)
                <div class="mb-2">
                    @php
                        ...
                        $categoryName = is_object($product->category) ? $product->category->name : ($product->category === 'instan' ? 'Produk Instan' : 'Produk Custom');
