<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-4 text-white">
            ...
            <div class="text-2xl font-bold">{{ number_format($banners->sum('views_count')) }}</div>
        </div>
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg p-4 text-white">
            <div class="text-sm opacity-90">Total Clicks</div>
            <div class="text-2xl font-bold">{{ number_format($banners->sum('clicks_count')) }}</div>
        </div>
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg p-4 text-white">
            ...
            @endphp
            <div class="text-2xl font-bold">{{ $avgCtr }}%</div>
        </div>
    </div>
                ...
                <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                    <div class="flex items-center flex-1 min-w-0"> <!-- Added min-w-0 -->
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium text-gray-900 truncate">{{ $banner->title ?? 'Untitled' }}</div>
                            <div class="text-xs text-gray-500">
                                <span class="bg-{{ $banner->type == 'home_banner' ? 'purple' : 'yellow' }}-100 text-{{ $banner->type == 'home_banner' ? 'purple' : 'yellow' }}-800 px-2 py-0.5 rounded-full">
                                    {{ $banner->type == 'home_banner' ? 'Home Banner' : 'Popup' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    <div class="text-right ml-4 flex-shrink-0">
                        <div class="text-sm font-bold text-gray-900">{{ number_format($banner->clicks_count) }}</div>
            ...
            <div class="space-y-3 max-h-80 overflow-y-auto"> <!-- Added max height and scroll -->
                ...
                <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-medium text-gray-900 truncate">{{ $banner->title ?? 'Untitled' }}</div>
                        <div class="text-xs text-gray-500">
                            <span>{{ number_format($banner->views_count) }} views</span>
                            <span class="mx-1">•</span>
                            <span>{{ number_format($banner->clicks_count) }} clicks</span>
                        </div>
                    </div>
                    <div class="text-right ml-4 flex-shrink-0">
                        <div class="text-sm font-bold text-{{ $ctrColor }}-600">
                            {{ $ctr }}%
                        </div>
                    ...
                    <tr>
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $banner->title ?? 'Untitled' }}</div>
                                    <div class="text-xs text-gray-500">{{ Str::limit($banner->description, 30) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs rounded-full {{ $banner->type == 'home_banner' ? 'bg-purple-100 text-purple-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $banner->type == 'home_banner' ? 'Home Banner' : 'Popup' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ number_format($banner->views_count) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ number_format($banner->clicks_count) }}</td>
                        <td class="px-6 py-4">
                            <span class="text-sm font-medium text-{{ $ctr > 5 ? 'green' : ($ctr > 2 ? 'yellow' : 'gray') }}-600">
                                {{ $ctr }}%
                            </span>
                        </td>
                        <td class="px-6 py-4">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const banners = @json($banners->sortByDesc('views_count')->take(10));
