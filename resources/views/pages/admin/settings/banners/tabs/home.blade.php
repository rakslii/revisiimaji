<div class="p-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('admin.settings.banners.create') }}?type=home_banner" 
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200 banners-list">
                @foreach($homeBanners->sortBy('display_order') as $banner)
                <tr data-id="{{ $banner->id }}" class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-sm text-gray-600">{{ $banner->subtitle ?? '-' }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-600">
                            @if($banner->start_date || $banner->end_date)
                                @if($banner->start_date)
                                    <div><span class="font-medium">Start:</span> {{ $banner->formatted_start_date }}</div>
                                @endif
                                @if($banner->end_date)
                                    <div><span class="font-medium">End:</span> {{ $banner->formatted_end_date }}</div>
                                @endif
                            @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center space-x-3">
                            <button onclick="toggleStatus({{ $banner->id }}, this)" 
                            ...
                            <button onclick="deleteBanner({{ $banner->id }}, '{{ addslashes($banner->title ?? 'Untitled') }}')" 
