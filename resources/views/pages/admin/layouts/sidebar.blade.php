<aside class="fixed left-0 top-0 h-screen w-64 bg-gray-900 text-white shadow-lg">
    ...
    <nav class="p-4 space-y-2">
        @foreach([
            ...
            ['route' => 'admin.orders', 'label' => 'Orders', 'icon' => 'fas fa-shopping-cart'],
            ['route' => 'admin.products', 'label' => 'Products', 'icon' => 'fas fa-box'],
            ['route' => 'admin.customers', 'label' => 'Customers', 'icon' => 'fas fa-users'],
            ['route' => 'admin.promo-codes', 'label' => 'Promo Codes', 'icon' => 'fas fa-tag'],
            ['route' => 'admin.settings', 'label' => 'Settings', 'icon' => 'fas fa-cog'],
        ] as $item)
            <a href="{{ route($item['route']) }}" 
               class="flex items-center gap-3 p-3 rounded-lg transition-colors {{ request()->routeIs($item['route']) ? 'bg-blue-600' : 'hover:bg-gray-800' }}">
