...
@section('title', 'Pembayaran - Cipta Imaji')
        ...
        <div class="container mx-auto px-4 py-8 md:py-12 relative z-10">
            <div class="max-w-6xl mx-auto">
                <nav class="flex items-center space-x-2 text-sm mb-6">
                    ...
                    <a href="{{ route('orders.index') }}" class="text-blue-200 hover:text-white transition-colors">
                        Pesanan Saya
                    </a>
                    <i class="fas fa-chevron-right text-blue-300 text-xs"></i>
                        ...
                        <div class="p-8 border-b border-gray-200">
                            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <h2 class="text-2xl font-bold text-gray-900">Order #{{ $order->order_code ?? $order->order_number }}</h2>
                                        <p class="text-gray-600 text-sm mt-2">
                                            <i class="far fa-clock mr-2"></i>
                                            Dibuat: {{ $order->created_at->translatedFormat('d F Y, H:i') }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col md:flex-row gap-4">
                                    <div class="px-5 py-3 bg-blue-100 text-[#193497] rounded-xl font-bold text-center min-w-[140px]">
                                        ...
                                        <p class="text-lg">{{ ucfirst(str_replace('_', ' ', $order->status)) }}</p>
                                    </div>
                                    <div class="px-5 py-3 {{ $order->payment_status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }} rounded-xl font-bold text-center min-w-[140px]">
                                        <p class="text-sm mb-1">Status Pembayaran</p>
                                        <p class="text-lg">{{ ucfirst($order->payment_status) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="flex items-center justify-between py-4 border-b border-gray-100 hover:bg-gray-50 px-3 rounded-lg transition-colors">
                                    <div class="flex items-center">
                                        ...
                                        @endif
                                        <div>
                                            <h4 class="font-bold text-gray-900 text-lg mb-1">{{ $item->product_name ?? $item->product->name ?? 'Product' }}</h4>
                                            <div class="flex items-center gap-4">
                                                <span class="text-gray-600 font-medium">
                                                    {{ $item->quantity }} × Rp {{ number_format($item->price, 0, ',', '.') }}
                                                </span>
                                                @if($item->product && $item->product->category_name)
                                ...
                                <div class="pt-6 space-y-4 bg-gray-50 p-6 rounded-2xl mt-4">
                                    ...
                                    <div class="flex justify-between items-center py-2">
                                        ...
                                        <span class="font-bold text-gray-900 text-xl">Rp {{ number_format($order->subtotal, 0, ',', '.') }}</span>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex justify-between items-center py-2">
                                        ...
                                        <span class="font-bold text-gray-900 text-xl">Rp {{ number_format($order->shipping_cost, 0, ',', '.') }}</span>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex justify-between items-center py-2">
                                        ...
                                        <span class="font-bold text-green-600 text-xl">- Rp {{ number_format($order->discount, 0, ',', '.') }}</span>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex justify-between items-center pt-6 border-t-2 border-gray-300">
                                        ...
                                        <span class="text-3xl font-bold text-[#193497]">
                                            {{ $order->formatted_total ?? 'Rp ' . number_format($order->total_amount ?? $order->total, 0, ',', '.') }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="text-center">
                                        ...
                                        <div class="bg-gradient-to-br from-[#193497] to-[#0f2354] rounded-2xl p-6 mb-6">
                                                ...
                                                @else
                                                    <div class="w-56 h-56 bg-gray-100 flex items-center justify-center rounded-lg">
                                                        <div class="text-center">
                                                            ...
                                                            <p class="text-sm text-gray-400 mt-1">{{ $order->order_code ?? $order->order_number }}</p>
                                                        </div>
                                                    </div>
                                                @endif
                                            </div>
                                            ...
                                            <div class="mt-6">
                                                ...
                                                <p class="text-blue-200 text-sm">Order #{{ $order->order_code ?? $order->order_number }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-8">
                                            ...
                                            <div class="text-4xl font-bold text-[#193497] mb-2" id="countdown-timer">
                                                {{ $payment->expired_at->diff(\Carbon\Carbon::now())->format('%H:%I:%S') }}
                                            </div>
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                ...
                                                @if($payment && $payment->qr_url && $payment->qr_url !== '#')
                                                <a href="{{ $payment->qr_url }}"
                                                   target="_blank"
