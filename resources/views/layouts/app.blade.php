...
<body class="font-sans bg-gray-50">
    ...
    @if(isset($popup) && $popup && $popup->is_valid)
    @php
        // Cek device
    ...
    <div id="promoPopup" 
         ...
         data-delay="{{ $popup->delay_seconds ?? 3 }}"
         data-once="{{ $popup->show_once_per_session ? 'true' : 'false' }}"
         style="background-color: rgba(0,0,0,0.7); backdrop-filter: blur(8px);">
        
        <div class="relative w-full max-w-{{ $popup->size == 'small' ? 'md' : ($popup->size == 'large' ? '4xl' : '2xl') }} transform transition-all duration-500 scale-95 opacity-0"
             id="popupContent"
             style="{{ $popup->background_style }}">
            ...
            <div class="relative overflow-hidden rounded-3xl shadow-2xl">
                ...
                <!-- Image -->
                @if($popup->image_url)
                <div class="relative cursor-pointer" onclick="window.location='{{ $popup->link ?? '#' }}'">
                    <img src="{{ $isMobile && $popup->mobile_image_url ? $popup->mobile_image_url : $popup->image_url }}" 
                         alt="{{ $popup->title }}"
                         class="w-full h-auto {{ $popup->size == 'full' ? 'max-h-screen' : 'max-h-[80vh]' }} object-cover">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex items-end">
                        <div class="p-8 text-white">
                            @if($popup->subtitle)
                            <p class="text-[#c0f820] font-semibold mb-2 text-lg">{{ $popup->subtitle }}</p>
                            @endif
                            
                            @if($popup->title)
                            <h3 class="text-4xl font-bold mb-3">{{ $popup->title }}</h3>
                            @endif
                            
                            @if($popup->description)
                            <p class="text-white/90 text-lg mb-4">{{ $popup->description }}</p>
                            @endif
                ...
                <div class="bg-white p-10 {{ $popup->size == 'full' ? 'min-h-screen' : '' }}" 
                     onclick="window.location='{{ $popup->link ?? '#' }}'"
                     style="cursor: pointer;">
                    <div class="text-center">
                        @if($popup->subtitle)
                        <p class="text-[#c0f820] font-semibold text-lg mb-3">{{ $popup->subtitle }}</p>
                        @endif
                        
                        @if($popup->title)
                        <h3 class="text-4xl font-bold text-gray-900 mb-4">{{ $popup->title }}</h3>
                        @endif
    ...
    function showPopup() {
        ...
        setTimeout(() => {
            ...
            // Track view
            fetch('/api/banners/{{ $popup->id }}/view', { 
                method: 'POST',
                headers: {
    ...
    function trackPopupClick() {
        fetch('/api/banners/{{ $popup->id }}/click', { 
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            }
